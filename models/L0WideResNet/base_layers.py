...
class MAPDense(Module):
    def __init__(self, in_features, out_features, bias=True, weight_decay=1.,args=None, **kwargs):
...
class MAPConv2d(Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                 weight_decay=1.,args=None, **kwargs):
    ...
    def prune(self,max_val,min_val,*kwargs):
        zero_matrix = torch.zeros_like(self.weight)
        self.weight = Parameter(
            torch.where(torch.gt(self.weight, min_val) & torch.lt(self.weight, max_val), zero_matrix, self.weight))
    ...
    def forward(self, input_):
        ...
        if self.args.isprune:
            adj = GetSubnet.apply(self.popup_scores.abs(), self.args.sparsity)
            weights_new = self.weight * adj
            output = F.conv2d(input_, weights_new, self.bias, self.stride, self.padding, self.dilation, self.groups)
            return output
        else:
