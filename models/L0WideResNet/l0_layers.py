...
class L0Dense(Module):
    """Implementation of L0WideResNet regularization for the input units of a fully connected layer"""
    def __init__(self, in_features, out_features, bias=True, weight_decay=1., droprate_init=0.5, temperature=2./3.,
                 lamba=1., local_rep=False, a=1,**kwargs):
        """
        :param in_features: Input dimensionality
        :param out_features: Output dimensionality
        :param bias: Whether we use a bias
        :param weight_decay: Strength of the L2 penalty
        :param droprate_init: Dropout rate that the L0WideResNet gates will be initialized to
        :param temperature: Temperature of the concrete distribution
        :param lamba: Strength of the L0WideResNet penalty
    ...
    def _reg_w(self):
        """Expected L0WideResNet norm under the stochastic gates, takes into account and re-weights also a potential L2 penalty"""
        q0=self.cdf_qz(0)
        logpw_col = torch.sum(- (.5 * self.prior_prec * self.weights.pow(2)) - self.lamba, 1)
        logpw = torch.sum(((self.a + 1) * (1 - q0) * logpw_col) / (self.a + (1 - q0) * logpw_col))
    ...
    def count_expected_flops_and_l0(self):
        """Measures the expected floating point operations (FLOPs) and the expected L0WideResNet norm"""
...
class L0Conv2d(Module):
    """Implementation of L0WideResNet regularization for the feature maps of a convolutional layer"""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                 droprate_init=0.5, temperature=2./3., weight_decay=1., lamba=1., local_rep=False,a=1, args=None,**kwargs):
        """
        ...
        self.popup_scores=Parameter(torch.Tensor(self.weights.shape))
    ...
    def prune(self,max_val,min_val,*kwargs):
        zero_matrix=torch.zeros_like(self.weights)
        self.weights=Parameter(torch.where(torch.gt(self.weights,min_val)&torch.lt(self.weights,max_val),zero_matrix,self.weights))
    ...
    def forward(self, input_):
        ...
        if not self.training:
            if self.args.isprune:
                adj = GetSubnet.apply(self.popup_scores.abs(), self.args.sparsity)
                weights_new = self.weights * adj
                output = F.conv2d(input_, weights_new, b, self.stride, self.padding, self.dilation, self.groups)
            ...
            if self.args.isprune:
                weights = self.sample_weights()
                adj=GetSubnet.apply(self.popup_scores.abs(),self.args.sparsity)
                weights_new=weights*adj
                output = F.conv2d(input_, weights_new, None, self.stride, self.padding, self.dilation, self.groups)
                return output
            else:
                weights=self.sample_weights()
